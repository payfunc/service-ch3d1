...
async function check(key: authly.Token, merchant: model.Merchant.Key.KeyInfo, token: authly.Token): Promise<api.check.Response | api.check.Error | gracely.Error> {
	return !merchant.card ? gracely.client.unauthorized() : api.check.post({ url: merchant.card.url, key }, {}, token)
}

async function enrolled(key: authly.Token, merchant: model.Merchant.Key.KeyInfo, request: api.enrolled.Request, token: authly.Token): Promise<api.enrolled.Response | gracely.Error> {
...
export class Verifier extends model.PaymentVerifier {
	...
	public async verify(key: authly.Token, request: model.PaymentVerifier.Request, force?: boolean): Promise<model.PaymentVerifier.Response> {
		...
		if (!merchant)
			...
			let token: authly.Token | gracely.Error | undefined = request.payment.type == "account"
				? request.payment.token
				: request.payment.type == "card"
				? request.payment.account ?? request.payment.card ?? request.payment.reference
				: undefined
			if (!token)
				result = gracely.client.malformedContent("request.payment.card | request.payment.account", "model.Card.Token | model.Account.", "not a card or account token")
			else {
				...
				if (gracely.Error.is(token))
					...
					if (!cardToken)
						...
						if (!force) { // check
							...
							const currency = request.payment.type == "card" ? request.payment.currency ?? merchant.currency ?? "EUR" : "EUR"
							const decimals = isoly.Currency.decimalDigits(currency) || 0
							const amount = Math.round(model.Item.amount(request.payment.type == "card" ? request.payment.amount ?? request.items : request.items) * 10 ** decimals)
							const enrolledRequest: api.enrolled.Request = {
								amount: amount.toString(),
								currency,
								order_id: api.enrolled.Request.orderIdLimit((request.reference.number ?? request.reference.id ?? "0").toString()),
								cardholder_ip: request.client.ip ?? "",
							}
							const enrolledResponse = await enrolled(key, merchant, enrolledRequest, token)
							rawResponse = enrolledResponse
							result = gracely.Error.is(enrolledResponse)
								? enrolledResponse
								: api.enrolled.Response.isOk(enrolledResponse)
								? model.PaymentVerifier.Response.verificationRequired(true, "GET", enrolledResponse.acs_url, { pareq: enrolledResponse.pareq })
