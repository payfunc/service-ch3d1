...
async function check(key: authly.Token, merchant: model.Merchant.Key, request: api.check.Request, token: authly.Token): Promise<api.check.Response | gracely.Error> {
	return (!merchant.card || merchant.card.emv3d?.protocol != "ch3d1") ? gracely.client.unauthorized() : api.check.post({ url: merchant.card.url, key }, request, token)
}
